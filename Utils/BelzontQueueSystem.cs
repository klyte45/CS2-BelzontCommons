...
namespace Belzont.Utils
{
    public abstract partial class BelzontQueueSystem<T> : SystemBase
    {
        ...
        {
            if (BasicIMod.TraceMode) LogUtils.DoLog("{0}: E = {1}, Force = {2}", GetType(), e, force);
        ...
        protected sealed override void OnUpdate()
        {
            ...
            {
                if (BasicIMod.TraceMode) LogUtils.DoTraceLog("{0}: ADVANCE/FORCE UNQUEUE!", GetType());
        ...
        internal void AddDependency(JobHandle other)
        {
            Dependency = JobHandle.CombineDependencies(Dependency, other);
        }
